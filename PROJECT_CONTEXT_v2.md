# Outreach Agent — Project Context for Claude Chat (v2)

## What This Is

A local Python app that automates my job search like a sales pipeline. It sources jobs, finds contacts at target companies, generates personalised outreach messages (10 A/B tested variants), manages LinkedIn + email outreach, tailors CVs per role, and tracks everything through an enterprise-level web dashboard with CRM-style contact timelines.
...
- Dashboard running at localhost:8000 with auth system + Chart.js visualisations
- CRM-style contact detail pages with activity timelines
...
- **Frontend:** Jinja2 templates, Tailwind CDN, Chart.js 4.4.1, vanilla JS
...
- **Auth:** Session-based with bcrypt (passlib) + itsdangerous signed cookies
...
**users** — Dashboard auth. Key fields: username, email, password_hash, full_name, settings (JSON blob for per-user config including API keys, search prefs, outreach limits).
...
├── PROJECT_CONTEXT.md          # Chat-friendly context (v1)
├── PROJECT_CONTEXT_v2.md       # Chat-friendly context (v2 — this file)
├── src/
...
│   └── dashboard/
│       ├── app.py              # FastAPI app
│       ├── routes.py           # All routes + auth + settings + chat API + CRM timeline
│       └── templates/          # 11 Jinja2 templates:
│                               #   base, dashboard, jobs, contacts, contact_detail,
...
Light theme, sidebar navigation, Inter font, indigo/purple accent. Enterprise-level UI inspired by HubSpot/Salesforce. 11 pages:

- **Login/Register** — Session-based auth with bcrypt passwords, signed cookies (7-day expiry)
- **Dashboard** — KPI cards (jobs, contacts, drafts, sent, replies, referrals), 4 Chart.js charts (pipeline funnel bar, weekly activity line, variant performance grouped bar, outreach breakdown doughnut), daily schedule timeline (08:00-20:00 automation stages), today's activity panel, LinkedIn quota progress bars, top jobs by fit score
- **Jobs** — Table with search, filters (score/source/status), sort. LinkedIn + Indeed links, apply button, expandable JD, CV generator
- **Contacts** — Table with filters (priority/relationship/status/source), LinkedIn profiles, email links. Clickable names link to CRM detail page
- **Contact Detail** — CRM-style page per contact: avatar + header, priority/status badges, email/LinkedIn/variant info, target role card with fit score, activity summary stats, vertical timeline with colour-coded events (created/draft/approved/sent/followup/response), expandable message bodies, full message history
...
- **Settings** — Full config: API keys (13 fields with show/hide toggle), search preferences, outreach limits, A/B testing params, Tier 1 companies. Saves to DB (per-user JSON) + .env file (system-wide)
